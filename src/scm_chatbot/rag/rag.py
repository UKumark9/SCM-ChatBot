...
from scm_chatbot.rag.vector_store import (
    EmbeddingModel,
    VectorIndex,
...
try:
    import sentence_transformers  # noqa: F401 - availability probe; SentenceTransformerEmbedding does the real import
    import faiss  # noqa: F401 - availability probe; FaissFlatIndex does the real import
except ImportError:
...
class VectorDatabase:
    ...
    Depends on EmbeddingModel and VectorIndex abstractions (vector_store.py)
    rather than sentence-transformers/FAISS directly, so either can be
    swapped or mocked without changing this class (Dependency Inversion).
    """
    ...
    def initialize(self):
        """Build the embedding model and vector index, unless already injected"""
        try:
            ...
            if self.index is None:
                self.index = FaissFlatIndex(self.dimension)
    ...
    def add_documents(self, new_documents: List[Dict]):
        ...
        try:
            ...
            logger.info(
                f"✅ Added {len(new_documents)} documents. Total: {len(self.documents)}"
            )
    ...
    def save_index(self, path: str):
        ...
        try:
            ...
            self.index.save(str(index_path / "index.faiss"))
    ...
    def load_index(self, path: str):
        ...
        try:
            ...
            if self.index is None:
                self.index = FaissFlatIndex(self.dimension)
            self.index.load(str(index_path / "index.faiss"))
...
class RAGModule:
    ...
    def retrieve_context(self, query: str, use_query_expansion: bool = True) -> str:
        ...
        try:
            ...
            filtered_results = [
                ...
                for doc, score in results
                if score
                < self.similarity_threshold  # Lower distance = higher similarity
            ]
    ...
    def _expand_query(self, query: str) -> List[str]:
        ...
        if "delay" in query_lower:
            expanded.extend(
                ["product delay management policy", "delivery delay classification"]
            )
