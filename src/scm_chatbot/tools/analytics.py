...
class SCMAnalytics:
    ...
    def analyze_delivery_delays(self) -> Dict:
        ...
        try:
            ...
            analysis = {
                ...
                "delays_by_state": self.orders.groupby("customer_state")["is_delayed"]
                .mean()
                .to_dict(),
                "delays_by_month": self.orders.groupby("order_month")["is_delayed"]
                .mean()
                .to_dict(),
            }

            logger.info(
                f"Delay analysis completed: {analysis['delay_rate_percentage']:.2f}% delay rate"
            )

            if self.feature_store:
                self.feature_store.set(
                    "analytics", "delivery_delays", analysis, ttl=1800
                )
    ...
    def analyze_product_delays(
        self, product_id: Optional[str] = None, category: Optional[str] = None
    ) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Analyzing product-level delivery delays (product_id={product_id}, category={category})..."
            )

            # Merge orders with order items and products
            order_products = self.orders.merge(
                self.order_items, on="order_id", how="inner"
            ...
            if product_id:
                ...
                if "product_category_name" in order_products.columns:
                    filtered_data = order_products[
                        order_products["product_category_name"] == category
                    ]
            ...
            analysis = {
                ...
                "delay_rate_percentage": (
                    (len(delayed_orders) / len(filtered_data)) * 100
                    if len(filtered_data) > 0
                    else 0
                ),
                "average_delay_days": (
                    float(delayed_orders["delay_days"].mean())
                    if len(delayed_orders) > 0
                    else 0
                ),
                "max_delay_days": (
                    float(delayed_orders["delay_days"].max())
                    if len(delayed_orders) > 0
                    else 0
                ),
                "median_delay_days": (
                    float(delayed_orders["delay_days"].median())
                    if len(delayed_orders) > 0
                    else 0
                ),
            }
            ...
            if not product_id and not category:
                ...
                product_delays = (
                    ...
                    .agg({"is_delayed": ["sum", "count", "mean"]})
                    .reset_index()
                )

                # Handle multi-level columns from groupby
                product_delays.columns = [
                    "product_id",
                    "delayed_count",
                    "total_count",
                    "delay_rate",
                ...
                product_delays_filtered = product_delays[
                    product_delays["total_count"] >= 5
                ].copy()
                logger.info(f"Products with 5+ orders: {len(product_delays_filtered)}")

                # Sort by delay rate
                product_delays_filtered = product_delays_filtered.sort_values(
                ...
                analysis["top_delayed_products"] = top_delayed_products.to_dict(
                    "records"
                )
                ...
                if "product_category_name" in order_products.columns:
                    ...
                    category_delays_filtered = category_delays[
                        category_delays["total_count"] >= 10
                    ].copy()
                    logger.info(
                        f"Categories with 10+ orders: {len(category_delays_filtered)}"
                    )

                    # Sort by delay rate
                    category_delays_filtered = category_delays_filtered.sort_values(
                        "delay_rate", ascending=False
                    )
                    top_delayed_categories = category_delays_filtered.head(10)

                    logger.info(f"Top delayed categories:\n{top_delayed_categories}")
                    analysis["top_delayed_categories"] = top_delayed_categories.to_dict(
                        "records"
                    )

            logger.info(
                f"Product delay analysis completed: {analysis['delay_rate_percentage']:.2f}% delay rate for {filter_label}"
            )
    ...
    def analyze_revenue_trends(self) -> Dict:
        ...
        try:
            ...
            # Monthly revenue
            orders_payments["year_month"] = orders_payments[
                "order_purchase_timestamp"
            ].dt.to_period("M")
            monthly_revenue = orders_payments.groupby("year_month")[
                "payment_value"
            ].sum()
            ...
            if len(revenue_list) > 1:
                growth_rates = [
                    (revenue_list[i] - revenue_list[i - 1]) / revenue_list[i - 1] * 100
            ...
            analysis = {
                ...
                "monthly_revenue": {
                    str(k): float(v) for k, v in monthly_revenue.to_dict().items()
                },
                ...
                "revenue_by_state": orders_payments.groupby("customer_state")[
                    "payment_value"
                ]
                .sum()
                .to_dict(),
            ...
            logger.info(
                f"Revenue analysis completed: Total revenue ${analysis['total_revenue']:,.2f}"
            )

            if self.feature_store:
                self.feature_store.set(
                    "analytics", "revenue_trends", analysis, ttl=3600
                )
    ...
    def analyze_product_performance(self) -> Dict:
        ...
        try:
            ...
            top_products = (
                product_sales.groupby("product_id")
            ...
            if "product_category_name" in product_sales.columns:
                category_sales = (
                    product_sales.groupby("product_category_name")
                    .agg({"order_id": "count", "price": "sum"})
            ...
            analysis = {
                ...
                "category_performance": (
                    category_sales.to_dict() if category_sales is not None else {}
                ),
            }

            logger.info(
                f"Product analysis completed: {analysis['total_unique_products']} unique products"
            )

            if self.feature_store:
                self.feature_store.set(
                    "analytics", "product_performance", analysis, ttl=3600
                )
    ...
    def analyze_customer_behavior(self) -> Dict:
        ...
        try:
            ...
            customer_orders = (
                ...
                .agg({"order_id": "count", "customer_state": "first"})
            ...
            customer_value = (
                self.orders.merge(
                    self.payments.groupby("order_id")["payment_value"]
                    .sum()
                    .reset_index(),
                    on="order_id",
                    how="left",
            ...
            analysis = {
                ...
                "repeat_customer_rate": (customer_orders["order_count"] > 1).mean()
                * 100,
                "average_customer_lifetime_value": customer_value.mean(),
                "customers_by_state": customer_orders.groupby("customer_state")
                .size()
                .to_dict(),
            ...
            logger.info(
                f"Customer analysis completed: {analysis['active_customers']} active customers"
            )

            if self.feature_store:
                self.feature_store.set(
                    "analytics", "customer_behavior", analysis, ttl=3600
                )
    ...
    def forecast_demand(
        self, product_id: Optional[str] = None, periods: int = 30
    ) -> Dict:
        ...
        try:
            ...
            daily_sales = (
                product_sales.groupby("date").size().reset_index(name="quantity")
            )
            daily_sales["date"] = pd.to_datetime(daily_sales["date"])
            daily_sales = daily_sales.sort_values("date")

            # Create time-based features
            daily_sales["days_since_start"] = (
            ...
            forecast_dates = [
                daily_sales["date"].max() + timedelta(days=i)
    ...
    def detect_inventory_risks(
        self, inventory_data: pd.DataFrame, low_stock_threshold: int = 100
    ) -> Dict:
        ...
        try:
            ...
            analysis = {
                ...
                "warehouse_distribution": inventory_data.groupby("warehouse_location")[
                    "current_stock"
                ]
                .sum()
                .to_dict(),
            ...
            logger.info(
                f"Inventory risk analysis completed: {len(low_stock)} low stock items"
            )
            return analysis
    ...
    def analyze_supplier_performance(self, supplier_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            analysis = {
                ...
                "average_on_time_delivery": supplier_data[
                ...
                * 100,
                ...
                "suppliers_by_location": supplier_data.groupby("location")
                .size()
                .to_dict(),
            }

            logger.info(
                f"Supplier analysis completed: {analysis['total_suppliers']} suppliers analyzed"
            )
            return analysis
