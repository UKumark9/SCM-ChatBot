...
from scm_chatbot.tools.forecast_chart_renderer import ForecastChartRenderer
...
class ForecastingEngine:
    ...
    Chart rendering (SRP: kept out of this class, see
    tools/forecast_chart_renderer.py) is delegated to ForecastChartRenderer.
    """
    ...
    def __init__(
        ...
        feature_store=None,
        chart_renderer: ForecastChartRenderer = None,
    ):
        ...
        self.chart_renderer = chart_renderer or ForecastChartRenderer()
    ...
    def _prepare_demand_series(self, history_months: int = 12) -> pd.Series:
        ...
        merged["week"] = (
            merged["date"] - pd.to_timedelta(dow, unit="D")
        ).dt.normalize()
    ...
    def _prepare_delay_rate_series(self, history_months: int = 12) -> pd.Series:
        ...
        df["order_delivered_timestamp"] = pd.to_datetime(
            df["order_delivered_timestamp"], errors="coerce"
        )
        ...
        weekly = df.groupby("week").agg(
            total=("is_late", "count"), late=("is_late", "sum")
        )
    ...
    def _prepare_category_series(
        ...
        merged = self.order_items.merge(
            self.orders[["order_id", "order_purchase_timestamp"]], on="order_id"
        ).merge(self.products[["product_id", "product_category_name"]], on="product_id")
        merged["date"] = pd.to_datetime(merged["order_purchase_timestamp"])
        ...
        cat_data["week"] = (
            cat_data["date"] - pd.to_timedelta(dow, unit="D")
        ).dt.normalize()
    ...
    def _run_sarima_on_series(
        ...
        try:
            ...
            logger.info(
                f"Walk-forward MAPE ({holdout_weeks}-week holdout): {mape:.2f}%"
            )

        except Exception as exc:
            logger.warning(
                f"Walk-forward MAPE failed: {exc}. Will use in-sample fallback."
            )
        ...
        # ── Step 4: Charts ───────────────────────────────────────────────────
        chart_b64 = self.chart_renderer.generate_chart(
            historical=series,
        ...
        bar_chart_b64 = self.chart_renderer.generate_forecast_bar_chart(
    ...
    def forecast_sarima(self, periods: int = 30) -> Dict[str, Any]:
        ...
        trend_arrow = {
            ...
            "stable": "Stable",
        }.get(result["trend"], result["trend"].title())
    ...
    def forecast_delay_rate(self, periods: int = 30) -> Dict[str, Any]:
        ...
        pie_b64 = self.chart_renderer.generate_pie_chart(
    ...
    def forecast_category(
        self, periods: int = 30, category: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def forecast_top_categories(
        self, periods: int = 30, top_n: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        for cat_name in top_cats.index:
            try:
                ...
                if len(cat_series) < 16:
                    logger.warning(
                        f"Skipping '{cat_name}': only {len(cat_series)} weeks"
                    )
                    continue
                ...
                try:
                    ...
                    pred = fv.get_forecast(steps=holdout_weeks).predicted_mean.clip(
                        lower=0
                    )
                    nz = test > 0
                    if nz.sum() > 0:
                ...
                logger.info(
                    ...
                    f"orders/wk, trend={trend}, MAPE={mape:.1f}%"
                    if mape
                    else f"Category '{cat_name}': avg={hist_mean:.0f} → {avg_forecast:.0f} orders/wk"
                )
        ...
        chart_b64 = self.chart_renderer.generate_multi_category_chart(
        ...
        bar_comp_b64 = self.chart_renderer.generate_category_comparison_bar(
            category_results, periods
        )

        # Pie chart of demand share
        pie_labels = [c.replace("_", " ").title()[:20] for c in category_results]
        pie_values = [res["avg_forecast"] for res in category_results.values()]
        pie_b64 = self.chart_renderer.generate_pie_chart(
            labels=pie_labels,
            values=pie_values,
