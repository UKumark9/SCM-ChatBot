...
class UIFormatter:
    ...
    def _format_metadata(
        agent: str, success: bool, used_rag: bool, classification: Dict, metrics: Dict
    ) -> str:
        parts = []
        ...
        agent_name = (
            agent.replace(" (Rule-Based)", "").replace("(Rule-Based)", "").strip()
        )
        ...
        if classification:
            ...
            if qtype:
                ...
                conf_label = (
                    "High"
                    if conf_pct >= 80
                    else ("Medium" if conf_pct >= 50 else "Low")
                )
                parts.append(qtype)
    ...
    def synthesize_rag_response(query: str, context: str, llm_client=None) -> str:
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                f"LLM synthesis failed, using raw context: {e}"
            )
            return UIFormatter.format_rag_context(context)
...
• Minor Delay: 1-2 business days""",
        ...
        "success": True,
        "used_rag": True,
        "classification": {"query_type": "policy", "confidence": 0.85},
        "metrics": {"documents_retrieved": 3, "execution_time": 1.82},
    }
