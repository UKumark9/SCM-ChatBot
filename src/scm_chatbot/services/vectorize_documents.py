...
def extract_pdf_text(file_path: Path) -> str:
    ...
    logger.warning(
        f"      ⚠️  No PDF extraction library available - using document name as content"
    )
...
def vectorize_uploaded_documents():
    ...
    try:
        ...
        from scm_chatbot.rag.rag import DocumentProcessor, VectorDatabase, RAGModule
        ...
        docs_to_vectorize = [
            doc for doc in all_docs if not doc.get("vectorized", False)
        ]
        ...
        vector_db = VectorDatabase(
            embedding_model_name="sentence-transformers/all-MiniLM-L6-v2", dimension=384
        )
        ...
        for idx, doc in enumerate(docs_to_vectorize, 1):
            ...
            try:
                ...
                dm.metadata["documents"] = [
                    d if d["id"] != doc["id"] else doc for d in dm.metadata["documents"]
                ]
                dm._save_metadata()
